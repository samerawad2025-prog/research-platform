...
| Confirmation page says "Add your research details" instead of reading the document | Not a bug by itself: the server is in manual mode. Check `EXTRACTION_MODE` in that deployment — missing or misspelled means `manual` on purpose, and the function log shows `extraction_mode_defaulted`. See `docs/deployment.md` "Rollout". | M1 |
