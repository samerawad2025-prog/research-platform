...
// 45s, not the 120s this used to default to.
//
// Two reasons, both arithmetic rather than taste.
//
// 1. It has to fit the function budget. maxDuration is 300s. The 503
//    ladder allows 3 attempts per pass with 2s + 9s of waits between
//    them, and a run can make two passes. At 120s per attempt the worst
//    case is 3 x 120 + 11 = 371s for pass 1 ALONE - over the ceiling,
//    so the platform would kill the function mid-run and strand the
//    paper in 'processing'. At 45s it is 3 x 45 + 11 = 146s per pass,
//    292s for both, which fits with room to spare.
//
// 2. It is still generous against reality. The slowest SUCCESSFUL
//    Gemini call ever measured on this project is 29.3s; the median is
//    around 16s. 45s leaves ~50% headroom over the worst real call.
//
// The practical effect for a submitter: when Gemini hangs (which it
// did twice on 2026-09-21, for the full 120s each time), they now wait
// 45 seconds to be told so, not two minutes.
...
// Retries only while the policy says another attempt could actually
// succeed. Bounded by construction: the policy caps the attempt count
// per failure class, so this loop cannot run away.
//
...
// stated delay and gets exactly one retry, because a retry spends more
// of the quota it just exhausted. A 503 carries no delay, costs nothing
// to retry, and gets an escalating ladder - one short retry was
// observed in production failing to clear a demand spike twice in under
// ten seconds (BUG_HISTORY.md #29, #36).
//
// This does NOT affect the two-provider-call ceiling the orchestrator
// enforces. That ceiling counts extraction PASSES. A 503 returns no
// content and is billed for nothing, so re-sending it is not a third
// extraction - it is the same pass still trying to happen.
async function callGemini(parts, context) {
  ...
  for (;;) {
    attempt += 1
    try {
      const result = await callGeminiOnce(parts, { ...context, attempt })
      if (attempt > 1) {
        result.diagnostics = {
          ...result.diagnostics,
          retriedAfter: lastError?.code ?? null,
          attempts: attempt,
        }
        console.log(JSON.stringify({
          stage: 'gemini_retry_succeeded',
          pass: context.pass,
          afterReason: lastError?.code ?? null,
          attempts: attempt,
        }))
      }
      return result
      ...
      if (!decision.retry) {
        ...
        // first diagnosed after the fact.
        console.log(JSON.stringify({
      ...
      console.log(JSON.stringify({
        ...
        attemptsRemaining: decision.attemptsRemaining ?? null,
        statedDelayMs: decision.statedDelayMs ?? null,
      }))
