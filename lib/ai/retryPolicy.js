...
// 503 means Google is overloaded ("This model is currently
// experiencing high demand"). A retry is cheap: the failed call
// consumed no quota and produced no tokens, and the condition is
// genuinely transient.
//
// Observed in production 2026-09-20: two real submissions failed with
// attempt 1 -> 503, a ~2s wait, attempt 2 -> 503, give up. Total elapsed
// under 10 seconds. A demand spike lasts minutes, not two seconds, so a
// single short retry was never going to clear one - it just failed
// twice quickly and told the submitter their document was unreadable.
//
// Escalating waits, and one more attempt than before. The whole ladder
// is bounded and cheap: worst case adds ~12s to a request that has a
// 300s ceiling, and costs nothing because a 503 is billed for nothing.
const OVERLOAD_BACKOFF_MS = [2000, 9000]
const MAX_OVERLOAD_ATTEMPTS = OVERLOAD_BACKOFF_MS.length + 1
...
//
// Sized to the PER-MINUTE free-tier window, which is what real
// submissions actually hit. Production, 2026-09-20: six submissions in
// seven minutes exhausted
// `generativelanguage.googleapis.com/generate_content_free_tier_requests`
// (limit: 20), and Google replied "Please retry in 35.260544627s". The
// cap was 30s, so the wait was refused by 5.26 seconds and a good
// Arabic DOCX failed outright - even though that window resets inside a
// minute and the retry would almost certainly have succeeded.
//
// 60s covers any per-minute reset with margin, and still fails fast on
// a daily quota, which returns hours and is nowhere near this. It stays
// far inside the route's 300s ceiling: a 429 is a rejection, not a
// generation, so it arrives in well under a second.
...
// retry is worth it.
// `attempt` is 1-based: the attempt that just FAILED. `alreadyRetried`
// is still accepted as the old boolean spelling of `attempt >= 2`.
function decideRetry({ status, pass, headers, body, alreadyRetried, attempt }) {
  const failedAttempt = Number.isFinite(attempt) ? attempt : alreadyRetried ? 2 : 1

  if (status === 503) {
    // A transient capacity problem, not a quota problem, so more than
    // one retry is affordable and is the only thing that actually
    // rides out a spike.
    if (failedAttempt >= MAX_OVERLOAD_ATTEMPTS) {
      return { retry: false, reason: 'overloaded_attempts_exhausted' }
    }
    return {
      retry: true,
      // Jitter spreads retries when several submissions hit the same
      // overload at once, so they don't all come back in lockstep and
      // re-collide on the same overloaded capacity.
      delayMs: withJitter(OVERLOAD_BACKOFF_MS[failedAttempt - 1]),
      reason: 'overloaded',
      attemptsRemaining: MAX_OVERLOAD_ATTEMPTS - failedAttempt - 1,
    }
  }

  // Everything below keeps the single-retry budget: a quota failure or
  // a dropped connection has no reason to behave differently on a
  // third go, and a 429 in particular spends quota to find out.
  if (failedAttempt >= 2) {
