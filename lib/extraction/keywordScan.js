...
const MARKERS = {
  ...
  // `title` had no markers at all, so a DOCX missing its title fell
  // straight through to the 12,000-character fallback slice rather
  // than a targeted excerpt. These are the labels that actually
  // precede a title on a thesis cover or an approval page; a title is
  // not always labelled, which is why the fallback still matters.
  title: ['title of the', 'thesis title', 'research title', 'title:', 'entitled'],
  title_ar: ['العنوان', 'عنوان البحث', 'عنوان الرسالة', 'بعنوان'],
  year: [],
};
