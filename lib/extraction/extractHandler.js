...
// Postgres "check_violation". Returned when the manual status write
// runs against a database that has not had migration 0011 yet.
...
// The only write is moving a still-pending paper to 'manual', so the
// record says honestly that nobody extracted it - and so a later switch
// back to automatic never quietly sends a document that its submitter
// was told they would describe by hand.
async function handleManual({ token, getSupabaseAdmin, log }) {
  ...
  try {
    ...
    // recorded, but the answer is the same and still sends nothing.
    return reply(200, { status: 'manual', mode: 'manual', recorded: false })
  }

  const { data: paper, error: lookupError } = await supabase
    .from('papers')
    .select('id, extraction_status, metadata_confirmed_at')
  ...
  if (paper.extraction_status !== 'pending') {
    // Processing, finished, failed or already manual: left exactly as
    // it is. A provider request already in flight cannot be recalled;
    // if it completes, its result is recorded and only applied if the
    // researcher has not confirmed by then (see the apply step below).
    return reply(200, { status: paper.extraction_status, mode: 'manual', recorded: false })
  }

  // Compare-and-swap, like every other status change here: only a row
  // that is still 'pending' moves.
  const { data: moved, error: moveError } = await supabase
    .from('papers')
    .update({ extraction_status: 'manual' })
    .eq('id', paper.id)
    .eq('extraction_status', 'pending')
    .select('id')

  if (moveError) {
    // Most likely migration 0011 has not been applied, so the CHECK
    // constraint rejects the value. The paper stays 'pending', which the
    // confirmation screen already treats as manual entry in this mode.
    // Loud, because it means the database is behind the code.
    log.warn(JSON.stringify({
      stage: 'manual_status_not_recorded',
      paperId: paper.id,
      pgCode: moveError.code ?? null,
      migrationMissing: moveError.code === CHECK_VIOLATION,
    }))
    return reply(200, { status: 'pending', mode: 'manual', recorded: false })
  }

  const recorded = Boolean(moved && moved.length)
  if (recorded) log.log(JSON.stringify({ stage: 'manual_entry_assigned', paperId: paper.id }))
  return reply(200, { status: recorded ? 'manual' : paper.extraction_status, mode: 'manual', recorded })
}
...
async function handleExtract({
  ...
  // 'manual' is deliberately not claimable: it is never 'pending', so it
  // falls through to here. A paper routed to manual entry stays there
  // even if the mode is later switched back to automatic.
  ...
  //
  // Every branch also requires metadata_confirmed_at to still be null,
  // so a confirmation that lands between the read above and this write
  // cannot be followed by a provider call.
  ...
  try {
    ...
    if (shouldApplyToPapers) {
      ...
      // never overwritten by a result that arrives late.
    ...
    if (!appliedToPapers) {
      ...
      if (shouldApplyToPapers) {
        log.log(JSON.stringify({ stage: 'extraction_result_not_applied_confirmed', paperId: paper.id }))
      }
    }
