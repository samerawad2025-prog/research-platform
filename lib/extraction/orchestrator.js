...
// Only these are worth a second, paid call on their own: for a real
// document, not_found on any of these is almost always a genuine
// extraction gap. Supervisor and abstract are deliberately excluded -
// many journal articles genuinely have no supervisor, and not every
// document has a clean abstract, so not_found for either is frequently
// the correct answer, not a gap worth chasing.
const CRITICAL_FIELDS = ['title', 'researchers', 'year'];
...
function getMissingCriticalFields(result) {
  return CRITICAL_FIELDS.filter((field) => isMissing(result[field]));
}
...
async function buildPass2Document(fileBuffer, fileType, fullText, missingFields) {
  if (fileType === 'pdf') {
    ...
    // chosen for in the first place. 25 pages, not 40: everything we
    // now ask about lives at the front of a document, never buried in
    // a middle chapter, so the earlier, wider allowance bought nothing.
