...
export async function POST(request) {
  ...
  try {
    ...
    // A final row for the merged view the confirmation screen reads,
    // distinct from either raw pass, so "which generation is live" is
    // never ambiguous.
    generationRows.push({
      paper_id: paper.id,
      generation_type: 'metadata_extraction',
      ...
      notes: `Merged result after ${extraction.passesRun} pass(es). Document type: ${extraction.documentType || 'unknown'}.`,
    })
    ...
    // submitter into a confirmation screen full of empty fields. The
    // merged generation still carries document_type, which is what lets
    // the confirmation page show a specific, useful message instead of
    // a generic failure.
    if (extraction.documentType === 'not_research') {
      await supabase
        .from('papers')
        .update({ extraction_status: 'failed', last_applied_generation_id: mergedGeneration.id })
        .eq('id', paper.id)

      return Response.json({ status: 'failed', reason: 'not_research' })
    }
    ...
    // (the AI operation itself succeeded); the papers columns and
