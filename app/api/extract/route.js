...
export async function POST(request) {
  ...
  try {
    ...
    // A configuration problem (most commonly: SUPABASE_SERVICE_ROLE_KEY
    // isn't set in this environment), not an extraction problem. Caught
    // explicitly so it's a clear, loggable 500 instead of an unhandled
    // crash - and so extraction_status is left exactly as it was
    // (still 'pending'), which means a later retry, once the
    // configuration is fixed, can simply try again rather than being
    // stuck behind a paper that looks like it's permanently mid-flight.
