...
export async function POST(request) {
  ...
  try {
    ...
    if (extraction.documentType === 'not_research') {
      await supabase
        ...
        .update({
          ...
          last_applied_generation_id: mergedGeneration.id,
        })
        .eq('id', paper.id)
    ...
    // The classification is always recorded, even when the metadata
    // itself isn't applied. It's how the confirmation page knows to
    // show "this doesn't look like an academic paper" instead of an
    // empty form or a generic failure.
    const papersPatch = {
