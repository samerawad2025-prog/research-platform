...
//
// This file fixes two real bugs found in production, both traced to
// the same root cause: there was no single place that consistently
// derived UI state from a fetch, and no real polling loop, only a
// one-shot check that fired on a status value the server leaves
// almost instantly. Now there is exactly one function that applies a
// fetch result to state, called by every poll tick, and one real
// polling loop that keeps checking until extraction actually finishes.
...
const MAX_POLL_ATTEMPTS = 40 // roughly 2 minutes at 3s apart, then give up automatically
...
function deriveResearchers(data) {
  ...
  return initial.length > 0 ? initial : [{ full_name: '', author_order: 1, linkedin_url: '', facebook_url: '' }]
}
...
export default function ConfirmationScreen({ token }) {
  ...
  useEffect(() => {
    ...
    // The single place that turns a fetch result into UI state. Called
    // on the first load AND on every poll tick, so there is exactly one
    // code path to keep correct, not two that can drift apart.
    function applyPaperData(data) {
      setPaper(data)

      const stillWorking = data.extraction_status === 'pending' || data.extraction_status === 'processing'

      // Seed the editable researcher list from whatever we have so far,
      // but only up until extraction actually finishes. Once it's
      // finished, lock it: the user may already be editing, and a
      // later tick (there shouldn't be one, but just in case) must
      // never overwrite what they've typed.
      if (!seededRef.current) {
        setResearchers(deriveResearchers(data))
    ...
    async function pollLoop(attempt) {
      ...
      if (!stillWorking) return // reached a final state, stop polling

      if (attempt === 0) {
        // First time we've observed a working state: make sure
        // extraction has actually been triggered at all. Safe to call
        // even if it's already running elsewhere - the server only
        // ever acts on a paper that's genuinely still 'pending'.
        fetch('/api/extract', {
          method: 'POST',
  ...
  if (paper.extraction_status === 'pending' || paper.extraction_status === 'processing') {
    return (
      <div className={styles.centered}>
        <p>We&rsquo;re reading through your paper now. This page will update on its own, usually within a minute, no need to refresh.</p>
        {pollTimedOut && (
          <>
            <p>This is taking longer than usual.</p>
            <button type="button" className={styles.addButton} onClick={() => window.location.reload()}>
              Try again
            </button>
          </>
        )}
    ...
    return (
      <div className={styles.completionPage}>
        ...
        <p className={styles.completionLead}>
          Your research details have been recorded accurately, in your own words, exactly as your paper presents them.
        </p>
        <p>
          Our team will now review your submission as it moves through the next stage of the platform&rsquo;s process.
          If we need anything further, we&rsquo;ll reach out using the details you provided.
        </p>
  ...
  return (
    <form onSubmit={handleConfirm} className={styles.page}>
      ...
      <section className={styles.section}>
        ...
        {['abstract', 'supervisor_name', 'year'].map((field) => (
