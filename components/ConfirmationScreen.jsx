...
export default function ConfirmationScreen({ token }) {
  ...
  async function retryExtraction() {
    ...
    try {
      ...
      // Safe to repeat: the server only ever acts on a paper that's
      // genuinely still 'pending', so retrying something already
      // completed or already mid-flight elsewhere is a no-op, not a
      // duplicate extraction.
      await new Promise((resolve) => setTimeout(resolve, 3000))
      const { data } = await supabase.rpc('get_paper_for_confirmation', { p_token: token })
  ...
  useEffect(() => {
    if (paper && paper.extraction_status === 'pending' && !retryAttempted) {
      ...
      <div className={styles.centered}>
        <p>
          {checking
            ...
            : 'We\u2019re still reading through your paper. This can take under a minute.'}
        </p>
        {retryAttempted && !checking && (
          <button type="button" className={styles.addButton} onClick={retryExtraction}>
            Try again
          </button>
        )}
      </div>
