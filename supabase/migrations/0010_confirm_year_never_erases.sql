-- 0010_confirm_year_never_erases.sql
--
-- Bug O (BUG_HISTORY.md #34): confirm_researcher_metadata ERASES a year
-- it cannot parse.
--
-- The live function body reads:
--
--   year = case
...
--          end
--
-- That `else null` is the whole bug. Send '٢٠١٩', '۲۰۱۹' or '٢٠١٩م'
-- and the pattern fails, so a correct year already sitting in the
-- column is replaced with null. The submitter typed a real year and
-- the database threw it away.
--
-- Not reachable from today's client, which normalizes to ASCII before
-- sending (BUG_HISTORY.md #20). This closes it at the layer that owns
-- the column, so it stays closed for any future caller.
--
-- Two changes, no more:
--
--   1. The accepted forms now match lib/extraction/applyResult.js
--      normalizeYear exactly, via a new normalize_year_text().
--   2. An unparseable year KEEPS the existing value instead of erasing
--      it. An empty string still clears the field deliberately, which
--      is how every other column in this UPDATE already behaves.
--
-- Everything else in confirm_researcher_metadata is reproduced
-- verbatim from schema.sql, unchanged. This file and schema.sql were
-- generated from the same text so the two cannot drift.
--
