...
| 0009 | `trigger_search_path.sql` | Pins `search_path = public` on `prevent_premature_publish()` | The last function in the schema without a fixed search_path, flagged by Supabase's own linter. `SECURITY INVOKER`, so lower risk than `BUG_HISTORY.md` #1, but closed for the same reason. `BUG_HISTORY.md` #35 |
| 0010 | `confirm_year_never_erases.sql` | Adds `normalize_year_text()`; replaces `confirm_researcher_metadata` so an unparseable year KEEPS the existing value instead of nulling it | `confirm_researcher_metadata` had an explicit `else null`, so `'٢٠١٩'`, `'۲۰۱۹'` and `'٢٠١٩م'` each erased a year the submitter had just confirmed. `normalize_year_text()` mirrors `lib/extraction/applyResult.js` `normalizeYear` exactly — Arabic-Indic and Persian digits folded to ASCII, 1900–2100, and exactly one in-range year or nothing. `BUG_HISTORY.md` #34 |
| 0011 | `manual_extraction_status.sql` | Widens `extraction_status` to allow `manual` | Phase 3 M1: a paper first seen while the server runs `EXTRACTION_MODE=manual` is recorded as manual entry, never as a completed extraction, and is not picked up for automatic extraction if the mode later changes. **Authored and tested against a local Postgres 16; not applied to production.** The app tolerates its absence (see the file header). |
