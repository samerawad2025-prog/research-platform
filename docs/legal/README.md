...
|---|---|---|---|
| §6: submitted content is not used for general-purpose AI model training; external AI extraction runs only under compatible provider arrangements | Either (a) the Gemini API project's data-use arrangement is verified as compatible, or (b) production actually runs with external extraction disabled for new submissions and uses the manual metadata path. M1 builds the switch that makes (b) possible. The switch is a fallback; it does not make the provider arrangement suitable, and building it does not meet this condition. | M1 (A) plus the verification or the production setting | **Unverified.** The API project's billing/data-use arrangement has not been checked. Historical rate-limit behaviour is not accepted as proof either way. |
