...
| extraction_status | `pending \| processing \| completed \| partial \| failed \| manual`. `partial` means pass 1 succeeded and its data is live, but pass 2 failed after its own retry — nothing was lost, some fields just weren't double-checked. `manual` (migration 0011, **not yet applied to production**) means no automatic extraction was attempted because the server was in `EXTRACTION_MODE=manual`; the researcher enters the details. It is never `completed`, and it is never picked up for extraction if the mode later changes. Until 0011 is applied, such a paper stays `pending`. A manually completed paper is recognisable as `metadata_confirmed_at` set with status `manual`, `pending` or `failed`. |
