...
const { sliceFrontMatter, extractTextLayer } = require('./pdf');
...
async function prepareDocument(fileBuffer, fileType, maxPages = 15) {
  ...
  if (fileType === 'pdf') {
    const [{ text }, sliced] = await Promise.all([
      extractTextLayer(fileBuffer),
      sliceFrontMatter(fileBuffer, maxPages),
    ]);
    return {
...
async function buildPass2Document(fileBuffer, fileType, fullText, missingFields) {
  ...
  // No usable text layer, or the scan found nothing: one bounded
  // broader attempt, not the whole document.
  if (fileType === 'pdf') {
    const broader = await sliceFrontMatter(fileBuffer, 40);
