...
- **`confirm_researcher_metadata(p_token, p_researchers jsonb, p_corrections jsonb default null)`** → `jsonb`. Token-gated write. Reconciles the researcher list (updates existing rows by id, inserts new ones, removes dropped ones — critically, this preserves the submitter's own email by matching on `researcher_id`, not by delete-and-recreate). Corrections use `p_corrections ? 'field'` to distinguish "the submitter deliberately cleared this" from "the submitter didn't touch this," not `coalesce`, which can't express clearing.
...
5. **`partial` extraction_status has never once occurred in production data**, despite the resilience-phase code existing. This needs a live check: is the resilience build actually deployed? (As of the last check during this handover, evidence pointed to *not fully deployed* — `failure_code` was null on every failed paper.)
6. **The most recent two fixes (header extraction, supervisor key normalization) have only been tested against mock data and reconstructed real-API-shaped responses.** Neither has been verified against a live Gemini call yet. This is the single highest-priority thing to confirm next.
...
9. **WhatsApp field is plain text**, not a proper country-picker/E.164 component. Deferred (Phase 4 of a broader UX redesign that was intentionally paused to focus on reliability).
...
- As of the last live snapshot: **26 papers completed, 10 failed, 6 stuck pending**, out of 42 total. The failed count includes both the 503-overload incident (now mitigated, unconfirmed whether the fix is deployed) and earlier pgcrypto/pdf-parse-era failures that predate all current fixes.
- The most recent code changes (merge filtering, excerpt widening, supervisor key fix, header extraction) are written, unit-and-mock-tested, and packaged, but **not yet confirmed against a live Gemini call.**
